...
class kitti_object(object):
    ...
    def __init__(self, root_dir, split="training", args=None):
        ...
        cam='cam6'
        self.split_dir=os.path.join(self.split_dir,cam)
        self.image_dir = os.path.join(self.split_dir, "image_1")
        ...
        # ####change
        self.label_dir = '/media/wuminghu/work/model/SMOKE/train/carla/inference/kitti_train/data'
...
def dataset_viz(root_dir, args):
    ...
    out_path='/media/wuminghu/work/output/SMOKE/test'
    ...
    if args.show_lidar_with_depth:
        ...
        fig = mlab.figure(
            ...
            # get camera intrinsic matrix cam_p
    with open(os.path.join(dataset.calib_dir, "%.6d.txt"%(0))) as csv_file:
        reader = csv.reader(csv_file, delimiter=' ')
        for line, row in enumerate(reader):
            if row[0] == 'P2:':
                cam_p = row[1:]
                cam_p = [float(i) for i in cam_p]
                cam_p = np.array(cam_p, dtype=np.float32).reshape(3, 3)
                cam_p = cam_p[:3, :3]
                break
    for data_idx in range(len(dataset)):
        ...
        if not os.path.exists(os.path.join(label_dir, "%06d.txt" % (data_idx))):
            continue
        ...
        if args.show_image_with_boxes:
            ...
            # import pdb; pdb.set_trace()
            vis_img=show_image_with_boxes(img, objects, cam_p, True, None)
            cv2.imwrite(os.path.join(out_path, "%06d.jpg" % (data_idx)), vis_img)
        # input_str = raw_input()
        # mlab.clf()
        # if input_str == "q":
        #     break
